...
跨命名空间共享是 relay 侧的一条 ACL 授予 `(owner, device, grantee)`。
一条授予等同于主人本人：relay 通过已认证的 agent 控制通道，给被共享方的会话盖上
和主人会话一样的完整能力集。`acl.perms` 这一列还在，写入常量 `full`，但没有任何
地方再去读它（ADR 0007）。被控端只跑一个绑定到一个账号的 agent，共享就是一台机器
拿到第二个使用者的方式；在这里再设一套更窄的能力集，等于在设备自己的权限模型之上
叠一套更弱的第二模型，而真正决定每一次请求的是设备那一套。

所以约束被共享方的是设备，不是这条授予。设备只有一份模式和一份规则，对所有人生效：
停在逐条确认，主人就要替对方的命令点头；开了 bypass，对方也跟着放行。提权执行对谁
都不在 bypass 覆盖范围内。设备控制台——审批、规则、模式、信任——额外要求控制端在
设备的 `portal_admins` 集合里，这是即使能力已经进了会话、控制面仍然留在主人手里的
原因。解绑设备和撤销共享仍然只有主人能做。在 ADR 0007 的门户那一半落地之前，
共享设备在门户里仍然是只读的。
