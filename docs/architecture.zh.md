...
可选的 [WebFetch 接入层](https://github.com/Daily-AC/wanctl/blob/main/docs/webfetch.md) 支持只会读取 URL 的网页 AI。申请、审批、
...
跨命名空间共享是 relay 侧的一条 ACL 授予 `(owner, device, grantee, manage)`。
一条共享继承主人对这台设备的**使用**权：relay 通过已认证的 agent 控制通道，给被共享方
的会话盖上 `UseCapabilities`——exec、read、write、logs。`acl.perms` 这一列还在，
写入常量 `full`，但没有任何地方再去读它（ADR 0007）。被控端只跑一个绑定到一个账号的
agent，共享就是一台机器拿到第二个使用者的方式；在这里再摆一张权限矩阵，等于在设备自己的
权限模型之上叠一套更弱的第二模型，而真正决定每一次请求的是设备那一套。

所以约束被共享方的是设备，不是这条授予。设备只有一份模式和一份规则，对所有人生效：
停在逐条确认，主人就要替对方的命令点头；开了 bypass，对方也跟着放行。提权执行对谁
都不在 bypass 覆盖范围内。

管理权——设备控制台：审批、规则、模式、受信控制端——是一条共享唯一可变的那一项，
由 `acl.manage`（migration 008）控制，主人不打开就是关的。打开后它给这个被共享方的
会话补上 `Console`，门户也会为他打开设备控制台。守着它的仍然是两道互不依赖的检查：
relay 这边的开关表示主人是否同意，设备那边的 `portal_admins` 集合表示哪个控制端身份
可以动手——所以即使开关打开，被共享方自己的控制端也不能直接管理这台设备。
解绑、改名、撤销共享，以及主人的通知设置，任何时候都不在共享范围内。

`/peers` 里 `devices` 依旧只是调用方自己的在线设备，含义不变；别人共享给他的设备另放在
`shared` 字段，带上主人 namespace、路由、标签和在线状态——否则被共享方根本无从知道
`--target` 需要的那个主人 namespace。不带 namespace 的 `--target` 先在调用方自己的
namespace 里解析，解析不到再去共享设备里找，按路由、名字或别名恰好命中一条才算数；
两个主人用了同一个标签时直接拒绝，不去猜。拒绝信息只在目标问的是调用方自己的
namespace 时才说出目标名；问的是没有授权的 namespace 时只回一个光秃秃的 `forbidden`，
不透露那边有没有这台设备。
