...
2. AI 返回 `approval_url` 和 `continuation_prompt`。你在自己的浏览器打开授权链接，登录
   wanctl，核对控制端和所选设备身份，选择设备及有效期。有效期给了 15 分钟、1 小时、
   4 小时、24 小时四个常用值，也可以自己填任意分钟数，上限 1440；要选得比这件事本身长，
   因为任务的截止时间会被授权结束时间截断。这同时也是 AI 手里那个会话链接的有效期，
   所以够用就好，做完及时吊销。读取授权链接本身不会批准申请。
...
| 工具 | 参数 | 结果 |
| --- | --- | --- |
| `exec` | `command`，可选 `cwd`、`timeout_seconds`（1–14400，默认 300） | 一次性执行、退出码和有限长度的 stdout/stderr |
...
限制：待审批申请 10 分钟过期；批准后有效期 1–1440 分钟、最多 16 台设备；每份授权按批准时长
每小时 64 个任务，不足一小时按一小时算，且不少于 64 个，所以 24 小时的授权有 1536 个。
`exec` 的 `timeout_seconds` 含排队为 1–14400 秒、默认 300 秒，编译、安装和渲染因此能跑完而不是
...
浏览器票据有不可延长的 1450 分钟上限：最长可批准的授权时长，加上票据走到批准那一步的
10 分钟。失效授权与任务内容在至少 25 小时后清理——严格长过上面那个上限，
所以旧票据不能重新创建已删除申请。原有账号与设备审计另行保留。

会话和状态链接是与授权同寿的凭证，最长可达一整天，不要公开仍有效的聊天记录；应先吊销授权再分享。
