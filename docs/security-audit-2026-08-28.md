...
| SEC-B-01 | High | A shared-device grantee could read owner-only console, logs, approval events, trust fingerprints, and notify email through the privileged portal. | **Fixed** in `785e818`: all four routes require ownership. **Narrowed** by ADR 0007 to shares without `manage`, and split three ways. Approvals, trust, rules and mode follow the per-share switch, off by default (`requireDeviceConsole`). The activity log and the approval event stream do not: they are reads of what the device did, every grantee already has them through `wanctl logs`, and gating them here would refuse in one client what the other allows (`requireDeviceUse`). The owner's notify and Feishu settings are contact details rather than device state and stay owner-only however the switch is set (`requireDeviceOwner`). The portal enforces all three server-side, because it dials with its own privileged token and the device cannot tell a grantee from the owner. |
...
| SEC-B-08 | Info | Admin ACL creation defaulted omitted permissions to `exec,read,write`, wider than the CLI default. | **Fixed** in `8cc1824`, then **retired** by ADR 0007: a grant carries no permission set, so there is no default to widen and `perms` is accepted and ignored. |
...
| SEC-C-02 | Medium | Shared-device portal reads exceeded ACL capabilities. | **Fixed** with SEC-B-01 in `785e818`. **Narrowed** by ADR 0007: a grant carries no capability subset to exceed, and what the portal shows a grantee is decided by the share's `manage` switch. |
