...
| SEC-B-01 | High | A shared-device grantee could read owner-only console, logs, approval events, trust fingerprints, and notify email through the privileged portal. | **Fixed** in `785e818`: all four routes require ownership. **Narrowed** by ADR 0007 to shares without `manage`: use-rights are inherited unconditionally, but the device's control plane is one per-share switch, off by default. The portal enforces it server-side in `requireDeviceConsole`, because it dials with its own privileged token and the device cannot tell a grantee from the owner. The owner's notify and Feishu settings are contact details rather than device state and stay behind `requireDeviceOwner` however the switch is set. |
