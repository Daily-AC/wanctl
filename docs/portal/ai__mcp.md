...
> 公网端点上第一次连某台设备时，AI 会收到一条「设备身份确认」——它自己调 `wanctl_trust_server` 把这次看到的指纹记下来，然后重试，不用问你。要你出面的是之后那一次：这台设备的指纹要是变了，连接当场断开，AI 会把记下的和这次看到的两个指纹都报给你等你判断，不会自己重记一遍。（这个开关由部署方在 relay 上打开，见自建文档里的「打开托管的 MCP 端点」；没打开就只能改用本机 stdio 接法。）
