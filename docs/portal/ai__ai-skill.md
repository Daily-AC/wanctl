...
> 一定要给 AI **relay 域**（`wanctl-relay.***REMOVED***...`）而不是门户域。门户域走飞书 SSO，AI 直接 fetch 会被登录页挡住，拿不到 skill 正文。

**b.** 让 AI 跑一次 `wanctl login`：会弹个浏览器让你飞书登录拿一次性 code，复制回去给 AI 回车即可——AI 这台机器就有你的身份了，可以用 `wanctl exec/push/pull` 控制你授权的设备。
