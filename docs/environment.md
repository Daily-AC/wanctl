...
| Variable | Role | Required | Default | Purpose |
|---|---|---:|---|---|
| `WANCTL_PORTAL` | relay, agent, controller | Conditional | persisted config, then build-time default | Portal URL used for login/enrollment and pairing links, and published by the relay as the MCP OAuth authorization endpoint. Persist with `wanctl config set portal=…`. |
