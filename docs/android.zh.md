...
1. 根据动画说明开启运行通知、允许后台持续运行。可以暂时跳过，但息屏连接和开机恢复可能受系统限制；稍后可在设置中补齐。
2. 选择**官方服务**或**自建服务**。官方服务与 CLI 使用同一组地址，目前需邀请；自建服务填写中继和门户地址。
3. 点击**使用 GitHub 登录**，浏览器完成授权后点击**返回 wanctl**。GitHub App 是否接管链接取决于其支持；不支持时使用浏览器。自建 SSO 门户可选其他登录方式，旧版门户可用授权码完成登录。
4. 首页点击**启用**。首页只显示连接状态和启用／停用按钮，配置会保留。

**设置**集中管理设备名、开机自启、通知与后台权限、服务地址、重新登录、日志和更新。
切换服务会停用当前连接、清除旧登录，再授权到新服务。提权通道默认关闭，在开启时才展示系统配置指引。

- **开机自动启用**默认开启，但只恢复用户已启用的设备，仍受系统自启动和电池策略限制。
- **自动信任新控制端**默认关闭；关闭时由门户确认新的控制端配对。
- **自动放行所有命令**默认关闭；关闭时继续使用正常命令审批策略。它和提权通道是独立设置。
- **设备名称**默认取机型，是可修改的显示标签。同型号设备可以同名；改名不会改变设备身份。

登录回跳要求门户也更新到支持移动端授权的版本。回跳只接受本机发起、十分钟内有效的登录状态；失败或取消可重新发起登录。

预览版不使用正式发布的应用内更新。设置中的「检查更新」会说明预览包的安装方式；同一包名、同一签名的新预览包可覆盖安装并保留配置。正式签名的发布包继续使用验签更新。
...
1. **控制端必须确认设备的身份。** `wanctl exec` 会打印一行
   `wanctl trust server --target … --fingerprint …`。把那个指纹跟 app 设置中「连接详情」的指纹
   显示的比一比——这一比就是全部意义所在，所以请用眼睛比，别去粘贴。
...
那个界面会显示一个端口和六位数字。保持弹窗打开，在另一台设备的浏览器中打开门户，选择这台手机的**设备设置 → ADB 配对**，填写端口和配对码。该入口需要 Android 客户端与门户均升级到 v0.6.0。
...
**提权命令是它们自己的一个策略类别，要两个开关都开着才会被 bypass 覆盖。** 规则是：

| 自动放行所有命令 | 提权通道 | 一条提权命令 |
|---|---|---|
| 开 | 开 | 直接跑，日志里记 `decision: bypass` 和跑它的那条通道 |
| 开 | 关 | 拒绝——没有通道，而且光有 bypass 不等于同意发 root |
| 关 | 都算 | 要一次批准，或者一条 `exec-elevated` 规则 |

两个开关默认都是关的，开哪一个都是一次明确的动作；
所以一台两个都开了的设备，主人说的是「无人值守」**并且**「要提权」，这就是那个决定。
一台开了 bypass 但没开提权通道的设备，照旧被拒绝。
这是 2026-09-18 主人拍的板（[#71](https://github.com/Daily-AC/wanctl/issues/71)）；
在那之前，一台无人值守手机上的提权命令在任何地方都授权不了。

在 normal 模式的设备上，一条没命中规则的提权命令走的是和普通命令同一条批准路径——
门户的设备控制台、设备自己的提示——而「允许并记住」会写下一条 `exec-elevated` 规则。
这条规则也可以提前写好：
...
一条 `exec` 规则从不授权同一条命令的提权形式，反过来也一样。

用 `--script` 发出去的命令在线上是一个 base64 大块，
所以设备改用一个稳定的记号来称呼它：`script:sh:<64 位十六进制>`，即脚本字节的 SHA-256。
记住的规则里存的是它，拒绝信息里打印的是它，
要预先授权某个脚本时 `--pattern` 里填的也是它。
它和那个大块一样窄——改一个字节就是另一个记号——
而且摘要是完整存的：这个记号**就是**那份授权，截断过的摘要是可以被碰撞的。
批准提示和门户卡片只显示前 16 位十六进制加一个 `…`，
那是给人看的标签，不是拿去抄的 pattern；完整记号在设备上的 `wanctl rules` 里。
只有真的是脚本的命令才会被缩写——长得像记号的普通命令一律原样显示，
因为一个把自己要问的东西藏掉一半的提示，比不提示还糟。

事件日志会记下每一条提权命令是走哪条通道跑的，
