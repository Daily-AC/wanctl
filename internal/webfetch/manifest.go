...
func securityRules() map[string]any {
	return map[string]any{
		...
		"retry_a_lost_response_on_the_same_url": "If a call's response never arrives, or a result read fails midway, fetch the IDENTICAL URL again with the SAME rid and the SAME arguments. That returns the job already recorded and can never run the operation twice. Changing any argument under a used rid returns 409. What comes back is the STORED result of that job, not a fresh one: a read replayed after the file changed still shows the text the first read returned, so read again under a new rid when you need the file as it is now.",
		"new_rid_only_when_nothing_ran":         "Use a new rid only after a result that says nothing ran: error_code pairing_required, adapter_busy or file_refused, each with execution_started: false. A file_refused read or edit did not replace the target file, so the corrected operation is a new one and needs a new rid. Never after a failed result without that flag, and never after unknown.",
